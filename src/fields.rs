...
This module provides parallel, multiple-bit, access to a `SliceBit`. This
functionality permits the use of `SliceBit` as a library-level implementation of
...
/** Permit a specific `SliceBit` to be used for C-style bitfield access.

Cursors that permit batched access to regions of memory are enabled to load data
from a `SliceBit` and store data to a `SliceBit` with faster behavior than the
default bit-by-bit traversal.

This trait transfers data between a `SliceBit` and an element. The trait
...
impl<T> BitField<T> for SliceBit<LittleEndian, T>
where T: BitStore {
	fn load(&self) -> Option<T> {
		let len = self.len();
		...
		match self.bitptr().domain() {
			...
			*/
			...
			_ => unreachable!(
				"Invalid memory representation! File an issue at https://github.com/sunjay/vecbit, and include this information:\n\
				Pointer: {:?}\n\
				Domain : {:?}",
				self.bitptr(),
				self.bitptr().domain(),
...
impl<T> BitField<T> for SliceBit<BigEndian, T>
where T: BitStore {
	fn load(&self) -> Option<T> {
		let len = self.len();
		if !(1 ..= T::BITS as usize).contains(&len) {
