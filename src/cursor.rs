...
`vecbit` structures are parametric over any ordering of bits within an element.
...
`vecbit` structures store and operate on semantic counts, not bit positions. The
