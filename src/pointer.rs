...
This module defines the binary representation of the handle to a `SliceBit`
region. This structure is crate-internal, and defines the methods required to
store a `SliceBit` pointer in memory and retrieve values from it suitable for
work.
!*/
...
/** In-memory representation of `&SliceBit` handles.
...
where T: BitStore {
	...
	/// Converts a `SliceBit` handle into its `BitPtr` representation.
	///
	/// # Parameters
	///
	/// - `bs: &SliceBit<C, T>`: a `SliceBit` handle
	///
	/// # Returns
	///
	/// The `BitPtr<T>` structure composing the handle.
	pub(crate) fn from_bitslice<C>(bs: &SliceBit<C, T>) -> Self
	where C: Cursor {
		let src = unsafe { &*(bs as *const SliceBit<C, T> as *const [()]) };
	...
	/// Converts a `BitPtr` structure into an immutable `SliceBit` handle.
	///
	/// # Parameters
	///
	/// - `self`
	...
	/// # Returns
	///
	/// A `SliceBit` handle composed of the `BitPtr` structure.
	pub(crate) fn into_bitslice<'a, C>(self) -> &'a SliceBit<C, T>
	where C: Cursor {
		unsafe {
			&*(slice::from_raw_parts(
				Pointer::from(self.ptr.as_ptr()).r() as *const (),
				self.len,
			) as *const [()] as *const SliceBit<C, T>)
		}
	}

	/// Converts a `BitPtr` structure into a mutable `SliceBit` handle.
	///
	/// # Parameters
	///
	/// - `self`
	...
	///
	/// A `SliceBit` handle composed of the `BitPtr` structure.
	pub(crate) fn into_bitslice_mut<'a, C>(self) -> &'a mut SliceBit<C, T>
	where C: Cursor {
		unsafe {
			&mut *(slice::from_raw_parts_mut(
				Pointer::from(self.ptr.as_ptr()).w() as *mut (),
				self.len,
			) as *mut [()] as *mut SliceBit<C, T>)
		}
	}
}
...
This is *unsafe* to use except from known mutable `SliceBit` structures.
...
impl<'a, C, T> From<&'a SliceBit<C, T>> for BitPtr<T>
where C: Cursor, T: 'a + BitStore {
	fn from(src: &'a SliceBit<C, T>) -> Self {
		Self::from_bitslice(src)
	}
}

impl<'a, C, T> From<&'a mut SliceBit<C, T>> for BitPtr<T>
where C: Cursor, T: 'a + BitStore {
	fn from(src: &'a mut SliceBit<C, T>) -> Self {
		Self::from_bitslice(src)
	}
}
