...
This module holds the type for an owned but ungrowable bit sequence. `VecBit` is
...
This type is essentially a `&SliceBit` that owns its own memory. It can change
the contents of its domain, but it cannot change its own domain like `VecBit`
...
`BitBox<C, T>` implements all the traits that `SliceBit<C, T>` does, by
deferring to the `SliceBit` implementation. It also implements conversion traits
to and from `SliceBit`, and to/from `VecBit`.
**/
#[repr(C)]
...
where C: Cursor, T: BitStore {
	...
	pub fn from_element(elt: T) -> Self {
		SliceBit::<C, T>::from_element(&elt).into()
	}
	...
	pub fn from_slice(slice: &[T]) -> Self {
		VecBit::from_slice(slice).into_boxed_bitslice()
	}

	/// Clones a `&SliceBit` into a `BitBox`.
	///
	/// # Parameters
	///
	...
	pub fn from_bitslice(slice: &SliceBit<C, T>) -> Self {
		VecBit::from_bitslice(slice).into_boxed_bitslice()
	}
	...
	pub fn from_boxed_slice(boxed: Box<[T]>) -> Self {
		...
		let bs = SliceBit::<C, T>::from_slice(&boxed[..]);
	...
	///   to even pass in `BitPtr<T>` values taken from `VecBit<C, T>` handles.
	///
	/// # Returns
	///
	...
	/// `&'a mut SliceBit<C, T>`. Note that the memory region `[T]` must outlive
	...
	///
	/// [`BitBox::from_raw`]: #method.from_raw
	pub fn leak<'a>(self) -> &'a mut SliceBit<C, T> {
		let out = self.bitptr();
		mem::forget(self);
	...
	///
	/// This delegates to `SliceBit::add_assign_reverse`.
	///
	/// # Parameters
	///
	/// - `self`
	...
	/// Accesses the `SliceBit<C, T>` to which the `BitBox` refers.
	///
	/// # Parameters
	///
	/// - `&self`
...
impl<C, T> Borrow<SliceBit<C, T>> for BitBox<C, T>
where C: Cursor, T: BitStore {
...
impl<C, T> BorrowMut<SliceBit<C, T>> for BitBox<C, T>
where C: Cursor, T: BitStore {
...
impl<A, B, C, D> PartialEq<SliceBit<C, D>> for BitBox<A, B>
where A: Cursor, B: BitStore, C: Cursor, D: BitStore {
...
impl<A, B, C, D> PartialEq<BitBox<C, D>> for SliceBit<A, B>
where A: Cursor, B: BitStore, C: Cursor, D: BitStore {
	fn eq(&self, rhs: &BitBox<C, D>) -> bool {
...
impl<A, B, C, D> PartialOrd<SliceBit<C, D>> for BitBox<A, B>
where A: Cursor, B: BitStore, C: Cursor, D: BitStore {
...
impl<A, B, C, D> PartialOrd<BitBox<C, D>> for SliceBit<A, B>
where A: Cursor, B: BitStore, C: Cursor, D: BitStore {
	fn partial_cmp(&self, rhs: &BitBox<C, D>) -> Option<Ordering> {
...
impl<C, T> AsMut<SliceBit<C, T>> for BitBox<C, T>
where C: Cursor, T: BitStore {
...
impl<C, T> AsRef<SliceBit<C, T>> for BitBox<C, T>
where C: Cursor, T: BitStore {
...
impl<C, T> From<&SliceBit<C, T>> for BitBox<C, T>
where C: Cursor, T: BitStore {
...
impl<C, T> From<VecBit<C, T>> for BitBox<C, T>
where C: Cursor, T: BitStore {
	fn from(src: VecBit<C, T>) -> Self {
